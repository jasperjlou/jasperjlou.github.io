...
“项目”里还有“金融助手”和“AI 同步助手”。分类选择 `AI + Tools · 多领域融合与工具实践` 后，项目会出现在项目页的这一组，也会自动显示在首页的工具列表中。

- “技术处理流程”控制示意图中的步骤，可以添加、修改和排序。
- “技术取舍”用来写实现方法和选择原因；“代码与验证链接”只填写访客能访问的公开链接。
- “源码可见性”选择 `public` 时显示仓库链接，选择 `private` 时显示私有说明和 GitHub 主页入口。
- 没有公开演示的项目可以把“产品网址”留空；展示形式选“技术处理流程”时，主图可以留空。
- “项目数字”只填写核验过的指标，没有数据可以保持空列表。

分类名称和副标题统一定义在 `src/data/project-groups.ts`。项目本身的介绍、流程和技术说明仍通过 TinaCMS 表单编辑。
